...
public class SubtitleFixer {
	...
	private void fixEncoding() throws IOException {
		...
		Charset ch = guessCharset(new BufferedInputStream(new FileInputStream(file)));
	...
	// windows-1251
	private void rewriteFile(File src, File dest, boolean usingCharSet) throws IOException {
		try (BufferedWriter writer = new BufferedWriter(new FileWriter(dest, false));
				BufferedReader br = usingCharSet == true
						? Files.newBufferedReader(Paths.get(src.getName()), Charset.forName("windows-1251"))
